...
BST_Node *newBST_Node(double freq, int bar, double index)
{
    ...
    if (bar < 0 || index > 1.0 || index < 0.0)
    {
        printf("Please Enter Valid Index's and Bar's\n");
        return NULL;
    }
...
void switchNode(BST_Node *root, int maxbar, int minbar)
{
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    reverseBST(root, maxbar(root), minbar(root));
    return root;
}
...
double NewFreq(double frequency, int semitones)
{
    for (int i = 0; i < 100; i++)
    {
        if ((note_freq[i] == frequency) && (i + semitones < 100) && (i + semitones >= 0))
        {
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    newFreq = NewFreq(root->freq, semitones);
    if ((newFreq != -1) && (0 <= root->index + time_shift) && (root->index + time_shift < 1))
    {

        new_node = newBST_Node(newFreq, root->bar, root->index + time_shift);
