...
int main()
{
  ...
  if (new_note == NULL)
    pass = 0;
  else if (new_note->freq != freq || new_note->bar != bar || new_note->index != index || new_note->key != (10.0 * bar) + index)
    pass = 0;
  if (pass == 1)
  ...
  root = BST_insert(root, new_note);
  if (root->left != new_note)
    pass = 0;
  if (root->left->key != (10.0 * bar) + index)
    pass = 0;
  bar = 2;
  index = 0.5;
  freq = 880.0;
  new_note = newBST_Node(freq, bar, index); // <- Where should this go in the tree?
  root = BST_insert(root, new_note);
  if (root->right != new_note)
    pass = 0;
  if (root->right->key != (10.0 * bar) + index)
    pass = 0;
  if (pass == 0)
  {
  ...
  if (t != NULL)
    pass = 0;
  if (root->left->bar != 1 || root->left->index != .75)
    pass = 0;
  if (pass == 0)
  {
