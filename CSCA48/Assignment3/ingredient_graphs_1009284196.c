...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
  ...
  if (ingredient_index(source_ingredient) == -1)
    return NULL;
  if (k_source < 0 || k_avoid < 0)
    return NULL;

  intNode *ingredients_total = related_k_dist(NULL, source_ingredient, k_source, 0);
  intNode *ingredients_avoid = related_k_dist(NULL, avoid, k_avoid, 0);

  intNode *h = NULL;
  intNode *curr = ingredients_total;
  while (curr != NULL)
  {
    if (searchInt(ingredients_avoid, curr->x) == 0 && curr->x != ingredient_index(avoid))
      h = insertInt(h, curr->x);
    curr = curr->next;
  }

  ingredients_total = deleteList(ingredients_total);
  ingredients_avoid = deleteList(ingredients_avoid);
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
  ...
  if (max != 0)
  {
    strcpy(recipe[replace_index], ingredients[max_index]);
  }
  else
  {
    strcpy(recipe[replace_index], "");
  }
}
