...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
    ...
    {
        printf("Duplicate node requested (bar:index) =%d,%1f, it was ignored\n:", new_node->bar, new_node->index);
        return root;
    }
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
     ...
     */
    if (root == NULL || (root->bar == bar && root->index == index))
    {
        return root;
    }

    if (root->bar > bar || (root->bar == bar && root->index > index))
    {
...
BST_Node *find_successor(BST_Node *right_child_node)
{
    ...
    {
        return NULL; // No successor for a null node
    }
    else if (right_child_node->left == NULL)
    {
        return right_child_node; // The right child node itself is the successor
    }
    else
    {
        // The successor is the node with the smallest key in the right subtree
        BST_Node *current_node = right_child_node->left;
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    else if (root->bar < bar || (root->bar == bar && root->index < index))
    {
...
int maxBar(BST_Node *root)
{
    BST_Node *current_node = root->right;
    while (current_node->right != NULL)
    {
        current_node = current_node->right;
    }
    return current_node->bar;
}
BST_Node *reverseSong(BST_Node *root)
{
    ...
    root->right = temp;
   //int max = maxBar(root->right);
    // Swap bar values with the swapped child nodes
    if (root->left != NULL)
    {
        // root->bar = abs(max - root->bar);
    }
    if (root->right != NULL)
    {
        //root->bar = abs(max - root->bar);
    ...
    // Update the index value of the current node
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    int skipLeft = root->left != NULL && root->left->index > 1 - time_shift;

    root->right = BST_harmonize(root->right, semitones, time_shift);
    int skipRight = root->right != NULL && root->right->index < -time_shift;

    if (!skipLeft && !skipRight && root->freq + semitones > 0 && root->freq + semitones < 99 &&
        root->index + time_shift >= 0 && root->index + time_shift < 1)
    {
        BST_Node *newNode = newBST_Node(root->freq + semitones, root->bar, root->index + time_shift);
